...
const useSidebarStore = (0,pinia__WEBPACK_IMPORTED_MODULE_2__.defineStore)('sidebar', () => {
  ...
  function open(node, tabId) {
    if (!(node && activeStore.activeFolder && activeStore.activeView)) {
      _logger_ts__WEBPACK_IMPORTED_MODULE_4__["default"].debug('sidebar: cannot open sidebar because the active folder or view is not set.', {
        node,
    ...
    if (isOpen.value && currentNode.value?.source === node.source) {
      _logger_ts__WEBPACK_IMPORTED_MODULE_4__["default"].debug('sidebar: already open for current node');
      if (tabId) {
        _logger_ts__WEBPACK_IMPORTED_MODULE_4__["default"].debug('sidebar: already open for current node - switching tab', {
          tabId
        });
        setActiveTab(tabId);
      }
    ...
    if (tabId && !newTabs.find(({
      ...
      _logger_ts__WEBPACK_IMPORTED_MODULE_4__["default"].warn(`sidebar: cannot open tab '${tabId}' because it is not available for the current context.`);
      activeTab.value = newTabs[0]?.id;
    } else {
      activeTab.value = tabId ?? newTabs[0]?.id;
    }
    _logger_ts__WEBPACK_IMPORTED_MODULE_4__["default"].debug(`sidebar: opening for ${node.displayname}`, {
      node
    });
  ...
  (0,_nextcloud_event_bus__WEBPACK_IMPORTED_MODULE_0__.subscribe)('viewer:sidebar:open', ({
    ...
    if (node) {
      _logger_ts__WEBPACK_IMPORTED_MODULE_4__["default"].debug('sidebar: opening for node from Viewer.', {
        node
      });
      open(node);
    } else {
      _logger_ts__WEBPACK_IMPORTED_MODULE_4__["default"].error(`sidebar: cannot open for node '${source}' because it was not found in the current view.`);
    }
  });
    ...
    if (!initialized) {
      ...
      window.OCP.Files.Router._router.afterEach((to, from) => {
        if (from.query && 'opendetails' in from.query && to.query && !('opendetails' in to.query)) {
          _logger_ts__WEBPACK_IMPORTED_MODULE_4__["default"].debug('sidebar: closing because "opendetails" query parameter was removed from URL.');
          close();
        }
      });
    }
  ...
  (0,vue__WEBPACK_IMPORTED_MODULE_3__.watch)(isOpen, isOpen => {
    ...
    _logger_ts__WEBPACK_IMPORTED_MODULE_4__["default"].debug(`sidebar: current node changed: ${isOpen ? 'open' : 'closed'}`, {
      query,
      params,
...
/******/ })()
;
//# sourceMappingURL=files-sidebar.js.map?v=e7290c8bc2a2524a9a81